...
from secret_agi.engine.game_engine import run_random_game, create_game


def test_game_completeness(num_games: int = 100, player_count: int = 5):
    ...
    for i in range(num_games):
        ...
        try:
            ...
            if result["completed"]:
                ...
                failed_games += 1
                print(f"    Game {i} failed to complete after {result['turns_taken']} turns")
    ...
    if turn_counts and max(turn_counts) > 500:
        ...
        print(f"✅ PASS: All games completed in reasonable time")
    ...
    if completed_games > 20:  # Only check balance if we have enough games
        if safety_wins == 0 or evil_wins == 0:
            print(f"❌ FAIL: Unbalanced outcomes (Safety: {safety_wins}, Evil: {evil_wins})")
            success = False
        else:
            print(f"✅ PASS: Both sides can win")
