...
from ..engine.models import GameState, ActionType, GameUpdate, Role
...
class RandomPlayer(BasePlayer):
    ...
    def __init__(self, player_id: str, seed: Optional[int] = None):
        """
    ...
    def choose_action(self, game_state: GameState, valid_actions: List[ActionType]) -> tuple[ActionType, Dict[str, Any]]:
        """
    ...
    def _generate_action_parameters(self, action: ActionType, game_state: GameState) -> Dict[str, Any]:
    ...
    def _generate_power_parameters(self, game_state: GameState) -> Dict[str, Any]:
        ...
        other_players = [p.id for p in game_state.alive_players if p.id != self.player_id]
    ...
    def on_game_start(self, game_state: GameState):
    ...
    def on_game_update(self, game_update: GameUpdate):
        ...
        if game_update.events:
            for event in game_update.events:
                if event.type.value in ['game_ended', 'paper_published', 'phase_transition']:
                    self.game_history.append({
                        "event": "update",
    ...
    def on_game_end(self, final_state: GameState):
...
class BiasedRandomPlayer(RandomPlayer):
    ...
    def choose_action(self, game_state: GameState, valid_actions: List[ActionType]) -> tuple[ActionType, Dict[str, Any]]:
        ...
        if ActionType.VOTE_TEAM in valid_actions or ActionType.VOTE_EMERGENCY in valid_actions:
            # Use biased voting but still random
            action_choice = random.choice([a for a in valid_actions if a != ActionType.OBSERVE] or valid_actions)
    ...
    def _generate_biased_vote_parameters(self, action: ActionType, game_state: GameState) -> Dict[str, Any]:
    ...
    def _choose_biased_paper(self, game_state: GameState) -> tuple[ActionType, Dict[str, Any]]:
        ...
        elif self.role in [Role.ACCELERATIONIST, Role.AGI] and self.role_bias.get("prefer_capability_papers"):
