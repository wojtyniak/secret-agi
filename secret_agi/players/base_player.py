...
from ..engine.models import GameState, ActionType, GameUpdate
...
class BasePlayer(ABC):
    ...
    def perform_action(self, action: ActionType, **kwargs) -> GameUpdate:
        """
...
class HumanPlayer(BasePlayer):
    ...
    def _get_action_parameters(self, action: ActionType, game_state: GameState) -> Dict[str, Any]:
        """Get parameters for the chosen action."""
        params = {}
        
        if action == ActionType.NOMINATE:
            eligible = [p.id for p in game_state.alive_players if not p.was_last_engineer]
    ...
    def on_game_start(self, game_state: GameState):
        ...
        for player in game_state.players:
            if player.id == self.player_id:
                self.role = player.role.value
                break
        
        # Find known allies (Accelerationists and AGI know each other)
        if self.role in ["Accelerationist", "AGI"]:
            for player in game_state.players:
                if player.id != self.player_id and player.role.value in ["Accelerationist", "AGI"]:
