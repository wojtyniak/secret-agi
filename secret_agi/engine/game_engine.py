...
from .models import (
    GameState, GameConfig, GameUpdate, Player, Paper, Role, 
    ActionType, Phase, create_standard_deck, get_role_distribution
)
from .actions import ActionProcessor, ActionValidator
from .rules import GameRules
from .events import GameStateManager, EventFilter
...
class GameEngine:
    ...
    def _create_players(self, config: GameConfig) -> List[Player]:
        ...
        for i, player_id in enumerate(config.player_ids):
            player = Player(
                id=player_id,
                role=roles[i],
                allegiance=roles[i].value if roles[i] != Role.SAFETY else "Safety"
            )
            players.append(player)
        
        return players
    
    def get_game_state(self, player_id: Optional[str] = None) -> Optional[GameState]:
        """
        ...
        if player_id:
            return EventFilter.filter_game_state_for_player(self._current_state, player_id)
        else:
    ...
    def perform_action(self, player_id: str, action: ActionType, **kwargs) -> GameUpdate:
        """
        ...
        # Process the action
        result = ActionProcessor.process_action(self._current_state, player_id, action, **kwargs)
        ...
        if result.game_state and result.success and self._current_state.get_player_by_id(player_id):
    ...
    def debug_get_full_state(self) -> Optional[GameState]:
        """
    ...
    def _generate_random_action_params(self, action: ActionType, player_id: str) -> Dict[str, Any]:
        ...
        if action == ActionType.NOMINATE:
            ...
            if self._current_state.director_cards:
                kwargs['paper_id'] = random.choice(self._current_state.director_cards).id
        
        elif action == ActionType.PUBLISH_PAPER:
            if self._current_state.engineer_cards:
                kwargs['paper_id'] = random.choice(self._current_state.engineer_cards).id
            ...
            alive_players = [p.id for p in self._current_state.alive_players if p.id != player_id]
            if alive_players:
...
def create_game(player_ids: List[str], seed: Optional[int] = None) -> GameEngine:
    """
...
def run_random_game(player_count: int = 5, seed: Optional[int] = None) -> Dict[str, Any]:
    """
