...
class PlayerDB(SQLModel, table=True):
    ...
    role: Role = Field()  # Safety, Accelerationist, AGI
    allegiance: Allegiance = Field()  # Safety, Acceleration
...
class Action(SQLModel, table=True):
    ...
    action_type: ActionType = Field()  # nominate, vote_team, etc.
...
class Event(SQLModel, table=True):
    ...
    game_id: str = Field(foreign_key="games.id", index=True)
    turn_number: int = Field(index=True)
    event_type: EventType = Field()  # state_changed, power_triggered, etc.
