...
from secret_agi.engine.models import (
    GameConfig, Player, Role, Allegiance, Phase, ActionType, Paper
)
...
class TestGameScenarios:
    ...
    def test_emergency_safety_mechanism(self):
        ...
        game_id = self.engine.create_game(config)
        ...
        if ActionType.CALL_EMERGENCY_SAFETY in valid_actions:
            ...
            result = self.engine.perform_action("safety1", ActionType.CALL_EMERGENCY_SAFETY)
            assert result.success
            
            # Safety players vote YES
            result = self.engine.perform_action("safety1", ActionType.VOTE_EMERGENCY, vote=True)
            assert result.success
    ...
    def test_role_distribution_correctness(self):
        ...
        for player_count in range(5, 11):
            ...
            assert roles.count(Role.SAFETY) >= roles.count(Role.ACCELERATIONIST) + roles.count(Role.AGI)
    ...
    def test_three_failed_proposals_auto_publish(self):
        ...
        initial_capability = state.capability
    ...
    def test_game_progresses_with_random_players(self):
        ...
        while not self.engine._current_state.is_game_over and turns < max_turns:
            ...
            if valid_actions:
                action, kwargs = players[acting_player].choose_action(state, valid_actions)
