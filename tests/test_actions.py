...
from secret_agi.engine.models import (
    GameState, Player, Paper, Role, Allegiance, Phase, ActionType
)
from secret_agi.engine.actions import ActionValidator, ActionProcessor
...
class TestActionValidator:
    ...
    def create_test_state(self, phase=Phase.TEAM_PROPOSAL):
        ...
        players = [
            ...
            Player("player2", Role.ACCELERATIONIST, Allegiance.ACCELERATION, alive=True),
...
class TestTeamProposalValidation:
    ...
    def create_team_proposal_state(self):
        ...
        players = [
            Player("director", Role.SAFETY, Allegiance.SAFETY, alive=True),
            Player("player2", Role.ACCELERATIONIST, Allegiance.ACCELERATION, alive=True, was_last_engineer=True),
            Player("player3", Role.AGI, Allegiance.ACCELERATION, alive=True),
        ]
    ...
    def test_nominate_validation(self):
        ...
        assert valid is False
        assert "Only the director" in error
        ...
        assert valid is False
        assert "already nominated" in error
    ...
    def test_emergency_safety_validation(self):
        ...
        assert valid is False
        assert "conditions not met" in error
        ...
        assert valid is False
        assert "already called" in error
    ...
    def test_vote_emergency_validation(self):
        ...
        assert valid is False
        assert "No emergency safety vote" in error
    ...
    def test_vote_team_validation(self):
        ...
        assert valid is False
        assert "No engineer nominated" in error
        ...
        assert valid is False
        assert "emergency safety vote first" in error
...
class TestResearchPhaseValidation:
    ...
    def create_research_state(self):
        ...
        players = [
            Player("director", Role.SAFETY, Allegiance.SAFETY, alive=True),
            Player("engineer", Role.ACCELERATIONIST, Allegiance.ACCELERATION, alive=True),
            Player("player3", Role.AGI, Allegiance.ACCELERATION, alive=True),
        ]
    ...
    def test_discard_paper_validation(self):
        ...
        assert valid is False
        assert "No papers to discard" in error
        ...
        assert valid is False
        assert "not in director's hand" in error
    ...
    def test_publish_paper_validation(self):
        ...
        assert valid is False
        assert "No papers to publish" in error
        ...
        assert valid is False
        assert "Only the engineer" in error
        ...
        assert valid is False
        assert "not in engineer's hand" in error
...
class TestActionProcessor:
    ...
    def create_test_state(self):
        ...
        from secret_agi.engine.models import Paper
        state.deck = [
            Paper("p1", 1, 1), Paper("p2", 2, 0), Paper("p3", 0, 2),
            Paper("p4", 1, 1), Paper("p5", 2, 0), Paper("p6", 0, 2),
            Paper("p7", 1, 1), Paper("p8", 2, 0), Paper("p9", 0, 2)
        ]
        return state
    ...
    def test_vote_team_success(self):
        ...
        ActionProcessor.process_action(state, "director", ActionType.VOTE_TEAM, vote=True)
        ActionProcessor.process_action(state, "player2", ActionType.VOTE_TEAM, vote=True)
        result = ActionProcessor.process_action(state, "player3", ActionType.VOTE_TEAM, vote=True)
    ...
    def test_vote_team_failure(self):
        ...
        # Majority vote no
        ActionProcessor.process_action(state, "director", ActionType.VOTE_TEAM, vote=True)
        ActionProcessor.process_action(state, "player2", ActionType.VOTE_TEAM, vote=False)
        result = ActionProcessor.process_action(state, "player3", ActionType.VOTE_TEAM, vote=False)
    ...
    def test_invalid_action_processing(self):
        ...
        assert "Only the director" in result.error
